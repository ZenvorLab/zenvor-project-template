...
- Agent A 通信：Telegram Bot（配置中），本地文件队列已可用。
- Agent B 执行器：Claude Code CLI（DeepSeek）+ Codex CLI，均配置自动审查。
- 用户消息链路：手机 GPT App（只读检查 GitHub，输出任务）→ 手动复制 → Telegram → Agent A → Agent B → GitHub → 下次 GPT 读取。
- Agent A 是多个项目的统一管理入口，用户只需与 A 对话即可管理 N 个项目的 Agent B。
