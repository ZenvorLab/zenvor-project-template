...
用户不在电脑前时，可通过协调 Agent (Agent A) 将任务转交给执行 Agent (Agent B)。详细规则见 [docs/project/REMOTE_AGENT_ORCHESTRATION.md](docs/project/REMOTE_AGENT_ORCHESTRATION.md)。

关键约束：

- Agent A 不修改 GPT 原始提示词，原样转发给 Agent B。
- Agent B 只执行当前任务提示词明确允许的修改。
- 每次任务执行完毕后，执行 Agent 自动 commit 并 push（默认），同时更新 `docs/project_status.md`。
- push、tag、force push、依赖安装、删除大量文件、修改权限、读取敏感数据等高风险操作必须确认。
- 通信 token、本地队列、logs、outputs 和临时产物不提交到业务仓库。
